...
void RandomizeRegisterInitPass::runOnOperation() {
  ...
  for (Operation &op : *getOperation().getBodyBlock())
  ...
  for (Operation *reg : regs) {
    auto regType = reg->getResult(0).getType().cast<FIRRTLType>();
    Optional<int64_t> regWidth = getBitWidth(regType);
    assert(regWidth.hasValue() && "register must have a valid FIRRTL width");

    auto start = builder.getIntegerAttr(ui64Type, width);
    auto end =
        builder.getIntegerAttr(ui64Type, width + regWidth.getValue() - 1);
    reg->setAttr("firrtl.random_init_start", start);
    reg->setAttr("firrtl.random_init_end", end);

    width += regWidth.getValue();
  }
